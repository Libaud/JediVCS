...
Version:      1.01
History:
Dec 28, 2004 V1.01 Added DeleteItem method and replaced code where needed
                   Added RemoveItemIfAged method
...
begin
    ...
    if FData.Count >= FMaxItems then begin
        DeleteItem(0);
{$IFDEF NEVER}
        PItemOld := FData.Items[0];
        TriggerDelete(PItemOld);
        // Free data object if associated properly
        if (PItemOld^.Data <> nil) and Assigned(PItemOld^.FreeFct) then
            PItemOld^.FreeFct(PItemOld^.Data);
        // Free record
        Dispose(PItemOld);
        FData.Delete(0);
{$ENDIF}
...
begin
    ...
    if FData.Count > FMaxItems then begin
        ...
        while FData.Count > FMaxItems do begin
            DeleteItem(0);
{$IFDEF NEVER}
            PItem := FData.Items[0];
            TriggerDelete(PItem);
